...
export function renderCard(item, idx) {
  ...
  if (mainImg) {
    if (hasMulti) {
      ...
      imgHtml = `<div class="relative aspect-square sm:aspect-[4/3] overflow-hidden bg-gray-100"><img class="w-full h-full object-cover card-img-zoom" src="${escA(mainImg)}" alt="" loading="lazy" onerror="this.parentElement.innerHTML='<div class=\\'flex items-center justify-center h-full text-4xl text-gray-300\\'>&#x1f3e0;</div>'"></div>`;
    }
  } else {
    imgHtml = `<div class="aspect-square sm:aspect-[4/3] bg-gray-100 flex items-center justify-center text-4xl text-gray-300">&#x1f3e0;</div>`;
  }
  ...
  return `<div class="card-wrap relative rounded-xl overflow-hidden bg-white border-2 border-transparent cursor-pointer transition-all hover:shadow-lg hover:border-gray-100" data-idx="${idx}" data-url="${escA(item.url || '')}" ${zameenIdAttr} ${areaAttr}>
    ...
    <div class="p-2 sm:p-3">
      ...
      ${item.url ? `<div class="card-action-row pt-1.5 sm:pt-2 mt-1.5 sm:mt-2 border-t border-gray-100">
        ...
        <div class="card-action-group">
          <a data-action="open" href="${escA(item.url)}" target="_blank" rel="noopener" class="action-btn w-8 h-8 rounded-full text-gray-400 hover:text-brand-500 hover:bg-brand-50 transition-colors" title="Open on Zameen.com" aria-label="Open on Zameen.com"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M18 13v6a2 2 0 01-2 2H5a2 2 0 01-2-2V8a2 2 0 012-2h6M15 3h6v6M10 14L21 3"/></svg></a>
          <button data-action="call" data-url="${escA(item.url)}" ${contactAttrs} class="action-btn w-8 h-8 rounded-full text-gray-400 hover:text-brand-500 hover:bg-brand-50 transition-colors" title="Call" aria-label="Call"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M3 5a2 2 0 012-2h3.28a1 1 0 01.948.684l1.498 4.493a1 1 0 01-.502 1.21l-2.257 1.13a11.042 11.042 0 005.516 5.516l1.13-2.257a1 1 0 011.21-.502l4.493 1.498a1 1 0 01.684.949V19a2 2 0 01-2 2h-1C9.716 21 3 14.284 3 6V5z"/></svg></button>
          <button data-action="whatsapp" data-url="${escA(item.url)}" ${contactAttrs} class="action-btn w-8 h-8 rounded-full text-gray-400 hover:text-green-600 hover:bg-green-50 transition-colors" title="WhatsApp" aria-label="WhatsApp"><svg viewBox="0 0 24 24" fill="currentColor"><path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347m-5.421 7.403h-.004a9.87 9.87 0 01-5.031-1.378l-.361-.214-3.741.982.998-3.648-.235-.374a9.86 9.86 0 01-1.51-5.26c.001-5.45 4.436-9.884 9.888-9.884 2.64 0 5.122 1.03 6.988 2.898a9.825 9.825 0 012.893 6.994c-.003 5.45-4.437 9.884-9.885 9.884m8.413-18.297A11.815 11.815 0 0012.05 0C5.495 0 .16 5.335.157 11.892c0 2.096.547 4.142 1.588 5.945L.057 24l6.305-1.654a11.882 11.882 0 005.683 1.448h.005c6.554 0 11.89-5.335 11.893-11.893a11.821 11.821 0 00-3.48-8.413z"/></svg></button>
        </div>
      </div>` : ''}
    </div>
  </div>`;
...
// Action icons — using filled SVGs (with viewBox 24×24) so visual weight
// stays consistent across all icons in the action row. The action-btn CSS
// rule forces every SVG inside to render at 16×16.
export const FAV_FILLED_SVG = '<svg viewBox="0 0 24 24" fill="currentColor"><path d="M12 21s-7-4.534-7-10a4 4 0 017-2.646A4 4 0 0119 11c0 5.466-7 10-7 10z"/></svg>';
export const FAV_HOLLOW_SVG = '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.2"><path stroke-linecap="round" stroke-linejoin="round" d="M12 21s-7-4.534-7-10a4 4 0 017-2.646A4 4 0 0119 11c0 5.466-7 10-7 10z"/></svg>';
export const HIDE_SVG = '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M13.875 18.825A10.05 10.05 0 0112 19c-7 0-10-7-10-7a16.94 16.94 0 014.169-5.249M9.88 5.083A10.052 10.052 0 0112 5c7 0 10 7 10 7a16.927 16.927 0 01-2.062 3.054M3 3l18 18"/></svg>';
export const COMPARE_SVG = '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M9 4v16M15 4v16M4 9h5M15 9h5M4 15h5M15 15h5"/></svg>';
